...
const StorefrontHeader = memo(function StorefrontHeader({
  user,
  theme,
  socialLinks,
}: {
  ...
  socialLinks: { key: string; url: string | null; Icon: React.ComponentType<{ className?: string }>; label: string }[];
}) {
  return (
    ...
    <section className="mb-10">
      <div className={cn("grid gap-4", layout === "featured" ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-2")}>
        {products.map((product) => {
          if (layout === "minimal") return <MinimalProductCard key={product.id} product={product} theme={theme} username={username} />;
          if (layout === "featured") return <FeaturedProductCard key={product.id} product={product} theme={theme} username={username} />;
          return <CompactProductCard key={product.id} product={product} theme={theme} username={username} />;
        })}
      </div>
    </section>
  );
            ...
            <div className="flex items-center gap-3">
              {link.iconUrl ? (
                <div className="relative h-6 w-6 overflow-hidden rounded-lg">
                  <Image src={link.iconUrl} alt="" fill className="object-cover" sizes="24px" />
                </div>
              ) : link.icon ? (
...
export default function StorefrontClient({ user, links, products }: Props) {
  const theme = useMemo(() => getTheme(user.themeName), [user.themeName]);
  ...
  const socialLinks = useMemo(
    ...
    [user.twitterUrl, user.instagramUrl, user.youtubeUrl, user.tiktokUrl, user.websiteUrl]
  );
  ...
  return (
    <div className={cn("min-h-screen", theme.bgColor, theme.textColor, theme.fontFamily)}>
      ...
      <div className="relative mx-auto max-w-2xl px-4 py-8 sm:py-12">
        <StorefrontHeader user={user} theme={theme} socialLinks={socialLinks} />
        <ProductGrid products={products} layout={layout} theme={theme} username={user.username} />
        <LinksSection links={links} theme={theme} userId={user.id} />
